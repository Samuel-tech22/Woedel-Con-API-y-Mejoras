...
// Array de palabras en mayúsculas para usar si la API no responde
const palabrasEnMayusculas = [
  "CIELO", "AGUA", "SOLAR", "LUNAR", "FUEGO", "TIERRA", "NUBES", "MONTA",
  "RIOJA", "MADER", "PIEDR", "AMIGO", "SILLA", "MESA", "TREN", "COCHE",
  "VIENT", "CALOR", "FRIO", "NIEVE", "LLAMA", "FRUTA", "VERDE", "AZUL",
  "ROJO", "BLANCO", "NEGRO", "GRIS", "PLATA"
];

// CONEXIÓN A LA API
const API = 'https://random-word-api.herokuapp.com/word?lang=es&length=5';

// Agrega un temporizador de 4 segundos para esperar la respuesta de la API
const apiPromise = new Promise((resolve, reject) => {
  fetch(API)
    ...
    .then((response) => {
      if (response.length === 0 || response[0].length !== 5) {
        // Si la API no devuelve una palabra válida, utiliza las palabras en mayúsculas
        word = palabrasEnMayusculas[Math.floor(Math.random() * palabrasEnMayusculas.length)];
        console.log(`Usando palabra en mayúsculas: ${word}`);
      } else {
        word = response[0].toUpperCase();
        console.log(`Palabra desde la API: ${word}`);
      }
      resolve();
    })
    .catch((error) => {
      // Si hay un error al cargar la API, elige una palabra al azar de todas formas.
      word = palabrasEnMayusculas[Math.floor(Math.random() * palabrasEnMayusculas.length)];
      console.log(`Error en la API. Usando palabra en mayúsculas: ${word}`);
      resolve();
    });

  // Espera 4 segundos antes de resolver la promesa
  setTimeout(() => {
    resolve();
  }, 4000);
});
...
apiPromise.then(() => {
  startGame(); // Llama a la función para iniciar el juego después de obtener la palabra o esperar el tiempo límite.
});
